...
def databaseToModel(element):
    return TimetableEvent(element[1], element[2], element[3], element[4], element[5], element[6], element[7], element[8], element[0])
...
def modelToJson(event: TimetableEvent):
    ...
    title = "{}, Sala: {}, Prowadzący: {}, Grupa: {}, {}".format(event.title, event.room, event.teacher, event.group, event.info)
